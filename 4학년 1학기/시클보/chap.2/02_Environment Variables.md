...
- 외부 프로그램을 통한 공격
	...
	- **system()** 
		...
		- 부모 프로세스의 환경 변수를 물려받는다 (일반 쉘에서 SUID 프로세스 실행 시 일반 쉘의 환경변수 물려받는다)
		- 쉘 프로세스로 prog_b 실행
	- **execve()**
...
- 일반 사용자가 root 권한 얻는 두가지 방식: Set-UID Approach vs Service Approach
	...
	- 서비스 접근 방식
		...
		- 안전
		- 시스템 오버헤드 존재(상시 실행중)
		- OS의 환경변수를 물려받으므로 일반 사용자가 개입할 수 없다
- 요약
...
- 환경 변수는 무엇인지
- 어떤 프로세스부터 다른 프로세스가 실행되는 상황에서 환경 변수가 어떤 식으로 전달될 수 있는지
- 환경 변수와 쉘 변수가 어떤 유사점과 차이점이 있는지
- 환경 변수와 관련된 다양한 리스크들
	- 라이브러리에서 환경변수 참조
	- 프로그램 코드 자체적으로 환경변수 참조시 새니타이제이션이 제대로 되는지
	- 