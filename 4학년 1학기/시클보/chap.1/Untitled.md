...
- SUID 공격 표면
	- 사용자 입력을 통한 공격
		- 명시적 입력 : 버퍼 오버플로우, 형식 문자열 취약점 이용해 아그이적 코드 실행
		- Change Shell과 같은 SUID 프로그램을 통해 사용자가 쉘 프로그램 변경 시, 입력값을 적절히 검증하지 않아 새로운 루트 계정 생성 가능
	- 시스템 입력을 통한 공격
	...
	- 환경변수를 통한 공격
		...
		- 사용자가 프로그램 실행 전 설정한 환경 변수를 통해 프로그램 동작에 영향