...
- 요약 : <font color="#00b0f0">Query Optimization</font>
...
- **Query Optimization**
	...
	- **비용 기반 Query Optimization의 과정**
		1. <font color="#00b0f0">Equivalence rule</font>을 사용해 논리적으로 동등한 표현식 생성
...
- 관계식의 변환
	...
	- 최소 Equivalence rule 집합
		- Equivalence rule 집합이 **최소**일 경우, 다른 규칙의 조합에서 어떤 규칙도 유도될 수 없음
		- 비최소 Equivalence rule 집합을 이용하면 표현식 생성 방법 수 증가
		- 쿼리 옵티마이저는 <font color="#00b0f0">최소 Equivalence rule 집합 사용</font>
	...
	- 동등한 표현의 열거
		- Query Optimization기는 주어진 표현식과 동등한 표현식을 <font color="#00b0f0">체계적으로</font> 생성하기 위해 Equivalence rule을 사용
		- 동등한 표현 생성 위해 다음을 반복
			- 지금까지 찾은 모든 동등한 표현의 모든 하위 표현식에 모든 적용 가능한 Equivalence rule을 적용
...
- 비용 추정 전략
	- 비용 추정을 위한 통계 정보
		...
		- 만약 r의 튜플이 물리적으로 파일에 함께 저장된다면
		  ...
		  예) n<sub>student</sub>=5000, f<sub>student</sub>=50 → b<sub>student</sub>= 100
	- 비용 추정을 위한 **근사 접근 방식**
...
- 평가 계획 선택
	- 평가 계획의 선택 
		...
		- <font color="#00b0f0">비용 기반 옵티마이저</font>는 주어진 쿼리와 동등한 모든 쿼리 평가 계획 공간을 탐색하고, 예쌍 비용이 가장 적은 계획을 선택
			- 복잡한 쿼리의 경우 모든 가능한 계획 공간을 탐색하는 것은 너무 많은 비용이 들 수 있으므로 대부분의 옵티마이저는 Query Optimization 비용을 줄이기 위한 <font color="#00b0f0">휴리스틱</font>을 포함
			- 이는 최적 계획을 찾지 못할 가능성이 있지만, Query Optimization 비용을 줄일 수 있음
	- 비용 기반 조인 순서 선택
